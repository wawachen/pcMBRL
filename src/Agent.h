...
namespace RVO {
	...
	class Agent {
	public:
		Agent(const Vector2 &position, float neighborDist, size_t maxNeighbors, float timeHorizon, float timeHorizonObst, float radius, float maxSpeed, const Vector2 &velocity = Vector2());
		~Agent();
		void setAgentPrefVelocity(const Vector2 &prefVelocity);
		void loadAgentNeighbors(const std::vector<Vector2> &positions, const std::vector<Vector2> &velocities);
		...
		const Vector2 &getAgentPosition() const;
                const std::vector<Line> &getAgentORCALines() const;
		/**
	...
	private:
		...
		// for loading neighbour information
