...
from pyrep.robots.mobiles.mobile_base1 import MobileBase
...
class NonHolonomicBase(MobileBase):
    ...
    def move(self, action):
        # thanks to Author: https://github.com/jfstepha/differential-drive/blob/master/scripts/twist_to_motors.py
        
        # dx = (l + r) / 2
        # dr = (r - l) / w
        dr = action[0]/3.49056 #angular velocity along z
        dx = action[1]/3.3#linear velocity along x, adjust parameter
        linear_v = [dx,0,0]
        # angular_v = [0,0,dr]

        vx = self.get_velocity()[0][0] #absolute velocity 
        # print("real",self.get_velocity()[0])

        m = self.get_matrix()
        m[3] = 0
        m[7] = 0
        m[11] = 0
        linearVa = sim.simMultiplyVector(m,linear_v) #absolute 

        # print("estimate",linearVa)
        # print(angularVa)

        ex = linearVa[0] - vx
        e_P = ex
        e_I = self.cummulative_error + ex
        e_D = ex - self.prev_error
        ad_vx = self.Kp * e_P + self.Ki * e_I + self.Kd * e_D


        # #need transform body to absolute velocity

        # self.cummulative_error = self.cummulative_error + ex
        # self.prev_error = ex

        # vright = 1.0 * ad_vx + dr * self.wheel_distance / 2 
        # vleft = 1.0 * ad_vx - dr * self.wheel_distance / 2
        vright = 1.0 * dx + dr * self.wheel_distance / 2 
        vleft = 1.0 * dx - dr * self.wheel_distance / 2


        left = vleft/self.wheel_radius
        right = vright/self.wheel_radius
