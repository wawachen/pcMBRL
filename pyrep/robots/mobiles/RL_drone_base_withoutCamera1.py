...
class RLDrone_base_nc1(Object):
    ...
    def __init__(self, count:int, num_propeller:int, name:str):
        ...
        self.frontR_sensor = ProximitySensor('Proximity_sensor_frontright'+suffix)
        self.frontL_sensor = ProximitySensor('Proximity_sensor_frontleft'+suffix)
        self.backR_sensor = ProximitySensor('Proximity_sensor_backright'+suffix)
        self.backL_sensor = ProximitySensor('Proximity_sensor_backleft'+suffix)
    ...
    def get_proximity(self):
        ...
        if self.front_sensor.read()[0] == -1:
            f = 0.0
        else: 
            f = self.front_sensor.read()[0]

        if self.frontR_sensor.read()[0] == -1:
            fr = 0.0
        else:
            fr = self.frontR_sensor.read()[0]

        if  self.right_sensor.read()[0] == -1:
            r = 0.0
        else:
            r = self.right_sensor.read()[0]
        
        if self.backR_sensor.read()[0] == -1:
            br = 0.0
        else:
            br = self.backR_sensor.read()[0]
        
        if self.back_sensor.read()[0] == -1:
            b = 0.0
        else:
            b = self.back_sensor.read()[0]

        if self.backL_sensor.read()[0] == -1:
            bl = 0.0
        else:
            bl = self.backL_sensor.read()[0]
        
        if self.left_sensor.read()[0] == -1:
            l = 0.0
        else:
            l = self.left_sensor.read()[0]

        if self.frontL_sensor.read()[0] == -1:
            fl = 0.0
        else:
            fl = self.frontL_sensor.read()[0]
        
        return [f,fr,r,br,b,bl,l,fl]
