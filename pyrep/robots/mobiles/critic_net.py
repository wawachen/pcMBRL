...
class CriticNetwork(network.Network):
  ...
  def __init__(self,
    ...
    (self.observation_spec, self.action_spec) = input_tensor_spec
  ...
  def call(self, inputs, step_type=(), network_state=(), training=False):
    ...
    outer_rank = nest_utils.get_outer_rank(observations, self.observation_spec)
    ...
    observations = self._encoder(observations, step_type=step_type, network_state=network_state)
    
    actions = tf.cast(tf.nest.flatten(actions)[0], tf.float32)
    #actions = tf.cast(tf.nest.flatten(actions), tf.float32)
    for layer in self._action_layers:
      actions = layer(actions, training=training)
    
    joint = tf.concat([observations[0], actions], 1)
