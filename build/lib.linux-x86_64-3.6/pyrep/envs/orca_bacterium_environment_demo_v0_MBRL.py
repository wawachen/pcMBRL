...
class Drone_Env:
    ...
    def __init__(self,args,env_name,num_agents):
        ...
        self.reward_callback = self.reward_and_terminate1
    ...
    def import_agent_models(self):
        ...
        if self.args.load_type == "eight":
            [m22,m2]= self.pr.import_model(path.join(robot_DIR, 'load_six.ttm'))
        if self.args.load_type == "twlve":
            [m22,m2]= self.pr.import_model(path.join(robot_DIR, 'load_six.ttm'))
    ...
    #This function is used for the training environment
    def step(self, action_n):
        ...
        for i, agent in enumerate(self.agents):
            #----------------------------
            rw,ter,dter = self.reward_and_terminate1(agent)
            reward_n.append(rw)
            done_n.append(ter)
    ...
    #This function is used for the evaluation environment
    ...
    #This function is used for the training environment
    def reward_and_terminate1(self, agent):
        rew = 0
        done_terminate = 0
        terminate = 0
    ...
    def observation(self):
        ...
        # return np.concatenate(all_pos+all_vel)
        ...
        # all_pos = np.array(all_pos)  # shape: (n, 2)
        # all_vel = np.array(all_vel)  # shape: (n, 2)
        ...
        for i, agent in enumerate(self.agents):
            ...
            for j, other_agent in enumerate(self.agents):
                ...
                other_vel = other_agent.get_2d_vel()
                ...
                obs.append(rel_pos)     # 2维: [rel_x, rel_y]
                obs.append(rel_vel)     # 2维: [rel_vx, rel_vy]
                # obs.append(distance)    # 1维: 距离
                # obs.append(bearing)     # 1维: 相对角度
            
            observations.append(np.concatenate(obs))
        
        return np.concatenate(observations)  # 返回长度为n的列表，每个元素是一个智能体的观测向量
