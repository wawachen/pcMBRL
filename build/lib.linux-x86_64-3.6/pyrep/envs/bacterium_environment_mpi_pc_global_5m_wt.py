...
class Drone_Env:
    ...
    def step(self, action_n):
        ...
        return np.array(obs_n), np.array(reward_n), np.array(done_n), np.array(done_g)
    ...
    def reward_and_terminate(self, agent):
        ...
        #team reward
        dists = [np.sqrt(np.sum(np.square(a.get_2d_pos() - self.goals[index,:2]))) for index, a in enumerate(self.agents)]
        finish_sig = np.all(np.array(dists) < 0.5)
        rew -= np.sum(np.array(dists))/(self.field_size*2*self.num_a)
