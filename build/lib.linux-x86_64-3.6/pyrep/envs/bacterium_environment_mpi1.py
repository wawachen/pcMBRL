...
class Drone_Env:
    ...
    def generate_goal(self):
        ...
        if self.num_a == 6:
            goal_points = np.array([[-2,0,1.7],[-4,0,1.7],[2,0,1.7],[4,0,1.7],[0,1.5,1.7],[0,3,1.7]]) #[-1.25,0,0.005],[0,1.25,0.005],[1.25,0,0.005]
        if self.num_a == 3:
            goal_points = np.array([[-2,0,1.7],[-4,0,1.7],[2,0,1.7]]) #[-1.25,0,0.005],[0,1.25,0.005],[1.25,0,0.005]

        if self.num_a == 12:
            goal_points = np.array([[-2,0,1.7],[-4,0,1.7],[2,0,1.7],[4,0,1.7],[0,1.5,1.7],[0,3,1.7],[0,-1.5,1.7],[0,-3,1.7],[-4,-3,1.7],[-4,3,1.7],[4,3,1.7],[4,-3,1.7]])
    ...
    def observation(self, agent):
        ...
        return np.concatenate([agent.get_2d_vel()] + entity_pos + other_pos)
