...
class Drone_Env:
    ...
    def __init__(self,args, env_name):
        ...
        self.agents = [Drone_s(i) for i in range(self.args.n_agents)]
        ...
        self.goal_index = np.random.choice(self.num_a)
    ...
    def generate_goal(self):
        ...
        for i in range(self.num_a):
            self.targets[i].set_position(goal_points[self.goal_index])
    ...
    def step(self, action_n):
        ...
        for i, agent in enumerate(self.agents):
            #----------------------------
            obs_n.append(self.observation_callback(self.goal_index,agent))
            rw,ter,dter = self.reward_callback(self.goal_index,agent)
    ...
    def reward_and_terminate(self, i, agent):
        ...
        #team reward
        dist = np.sqrt(np.sum(np.square(agent.get_2d_pos() - self.goals[i,:2])))
        finish_sig = (dist < 0.5)
    ...
    def local_observation(self, index, agent):
        ...
        entity_pos.append((self.goals[index,:2]-agent.get_2d_pos())/(self.field_size*2))   
        ...
        return np.concatenate([agent.get_2d_vel()]+ [agent.get_2d_pos()/self.field_size] + other_pos + other_live + entity_pos)
    ...
    def observation(self, index, agent):
        ...
        return np.concatenate([agent.get_2d_vel()]+ [agent.get_2d_pos()/self.field_size] + other_pos + entity_pos)
