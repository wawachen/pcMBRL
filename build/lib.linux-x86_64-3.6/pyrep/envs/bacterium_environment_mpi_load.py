...
class Drone_Env:
    ...
    def __init__(self,env_name,num_agents):
        ...
        self._collision_load = sim.simGetCollectionHandle(
            'load')
    ...
    def load_spread(self):
        ...
        vpt_o = random.uniform(-180.0,180.0)
        self.payload.set_position([vpt[0],vpt[1],0.1])
        self.payload.set_orientation([0.0,0.0,vpt_o])
        while sim.simCheckCollision(self._collision_load,sim.sim_handle_all) == 1:
            vpt = [random.uniform(self.x_limit_min,self.x_limit_max),random.uniform(self.y_limit_min,self.y_limit_max)]
            vpt_o = random.uniform(-180.0,180.0)
            self.payload.set_position([vpt[0],vpt[1],0.1])
            self.payload.set_orientation([0.0,0.0,vpt_o])
    ...
    def generate_goal_l(self):
        ...
        points = []
        if self.num_a == 3:
            points.append([self.payload_1.get_position()[0],self.payload_1.get_position()[1],1.5])
            points.append([self.payload_2.get_position()[0],self.payload_2.get_position()[1],1.5])
            points.append([self.payload_3.get_position()[0],self.payload_3.get_position()[1],1.5])
    ...
    def reset_world(self):
        ...
        if self.close_simulation:
            self.targets = [Shape.create(type=PrimitiveShape.CYLINDER,
                      ...
                      color=[1.0, 0.1, 0.1],
                      static=True, respondable=False) for i in range(self.num_a)]
        self.goals = self.generate_goal_l()
