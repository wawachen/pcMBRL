...
class Drone_Env:
    ...
    def direct_spread(self,state):
        ...
        pos = []
        for i in range(self.num_a):
            pos.append(state[0,i*self.observation_space[i].shape[0]+2:i*self.observation_space[i].shape[0]+4])
        pos = np.array(pos)
        ...
        for i in range(self.num_a):
            ...
            self.agents[i].agent.set_3d_pose([pos[i,0]*self.field_size,pos[i,1]*self.field_size,1.7,0.0,0.0,0.0])
                

        return model_handles,objs

    def reset_world_train(self,state):
        self.time_step = 0
        ...
        self.model_handles,ii = self.direct_spread(state)
        ...
        return np.concatenate(obs_n,axis=0)
    ...
    def step(self, action_n):
        ...
        return np.concatenate(obs_n,axis=0), np.array(reward_n), np.array(done_all), np.all(done_ter)
