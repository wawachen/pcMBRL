...
class SphericalVisionSensor(Object):
    ...
    def __init__(self, name, suffix):
        ...
        self._sensor_depth = VisionSensor('%s_sensorDepth' % (name)+suffix)
        self._sensor_rgb = VisionSensor('%s_sensorRGB' % (name)+suffix)

        # directed sub-sensors
        names = ['front', 'top', 'back', 'bottom', 'left', 'right']
        self._six_sensors = [VisionSensor('%s_%s' % (name, n)+suffix) for n in names]
