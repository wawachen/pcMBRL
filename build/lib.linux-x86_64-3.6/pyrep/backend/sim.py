...
def simFloatingViewAdd(posX, posY, sizeX, sizeY, options):
    return lib.simFloatingViewAdd(posX, posY, sizeX, sizeY, options)

def simAdjustView(viewHandleOrIndex, associatedViewableObjectHandle, options, viewLabel):
    return lib.simAdjustView(viewHandleOrIndex, associatedViewableObjectHandle, options, viewLabel)
...
def simSetLinkDummy(dummyHandle,dummyHandle1):
    lib.simSetLinkDummy(dummyHandle,dummyHandle1)
...
def simGetSystemTimeInMs(previousTime):
    return lib.simGetSystemTimeInMs(previousTime)
