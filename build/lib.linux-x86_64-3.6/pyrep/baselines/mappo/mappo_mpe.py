...
class MAPPO_MPE(nn.Module):
    def __init__(self, args):
        super(MAPPO_MPE,self).__init__()
        self.N = args.N
    ...
    def train(self, replay_buffer, total_steps,logger):
        ...
        for batch_index in range(self.batch_size):
            ...
            with torch.no_grad():  # adv and td_target have no gradient
                ep_limit = int(batch['duration'][batch_index].item())
                
                deltas = batch['r_n'][batch_index,:ep_limit] + self.gamma * batch['v_n'][batch_index, 1:ep_limit+1] * (1 - batch['done_n'][batch_index, :ep_limit]) - batch['v_n'][batch_index, :ep_limit]  # deltas.shape=(episode_limit,N)
                
                for t in reversed(range(ep_limit)):
                    gae = deltas[t] + self.gamma * self.lamda * gae
                    adv.insert(0, gae) #tiime_limit*[N]
                
                adv = torch.stack(adv)  # adv.shape(episode_limit,N)
                assert adv.shape[0] == ep_limit
                v_target = adv + batch['v_n'][batch_index, :ep_limit]  # v_target.shape(episode_limit,N)
                if self.use_adv_norm:  # Trick 1: advantage normalization
                    adv = ((adv - adv.mean()) / (adv.std() + 1e-5))
                adv_batch.append(adv) #batch*[episode_limit,N]
                v_target_batch.append(v_target) #batch*[episode_limit,N]
        ...
        for _ in range(self.K_epochs):
            for index in BatchSampler(SequentialSampler(range(self.batch_size)), self.mini_batch_size, False):
                ...
                """
                for mini_index in index:
                    ep_limit1 = int(batch['duration'][mini_index].item())
                    if self.use_rnn:
                        ...
                        probs_now = self.actor(actor_inputs[mini_index,:ep_limit1]) #[episode_limit,N,act_dim]
                        values_now = self.critic(critic_inputs[mini_index,:ep_limit1]).squeeze(-1) #[episode_limit,N]
                        # print(probs_now.shape)
                        # print(values_now.shape)

                    dist_now = Categorical(probs_now)
                    dist_entropy = dist_now.entropy()  # dist_entropy.shape=(episode_limit, N)
                    # print(dist_entropy.shape)
                    # batch['a_n'][index].shape=(mini_batch_size, episode_limit, N)
                    
                    a_logprob_n_now = dist_now.log_prob(batch['a_n'][mini_index,:ep_limit1])  # a_logprob_n_now.shape=(episode_limit, N)
                    # a/b=exp(log(a)-log(b))
                    ratios = torch.exp(a_logprob_n_now - batch['a_logprob_n'][mini_index,:ep_limit1].detach())  # ratios.shape=(episode_limit, N)
                    surr1 = ratios * adv_batch[mini_index]
                    surr2 = torch.clamp(ratios, 1 - self.epsilon, 1 + self.epsilon) * adv_batch[mini_index]
                    actor_loss = -torch.min(surr1, surr2) - self.entropy_coef * dist_entropy

                    logger.add_scalar('actor loss', actor_loss.mean().item(), total_steps)
                    logger.add_scalar('entropy', dist_entropy.mean().item(), total_steps)
                    logger.add_scalar('KL', ratios.mean().item(), total_steps)
                    ...
                    if self.use_value_clip:
                        ...
                        critic_loss = (values_now - v_target_batch[mini_index]) ** 2
                    
                    logger.add_scalar('critic loss', critic_loss.mean().item(), total_steps)
    ...
    def save_model(self, path, env_name, number, seed, total_steps):
        if not os.path.exists(path+"/model"):
            os.makedirs(path+"/model")
        torch.save(self.actor.state_dict(), path+"/model/MAPPO_actor_env_{}_number_{}_seed_{}_step_{}k.pth".format(env_name, number, seed, int(total_steps)))

    def load_model(self, path):
        self.actor.load_state_dict(torch.load(path+"/model/MAPPO_actor_env.pth"))
