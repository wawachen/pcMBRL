...
class Actor_att1(nn.Module):
    ...
    def forward(self, s_input, a_input = None, logger=None, agent_id=None, iter_n=None):
        ...
        #other mlp
        other_good_in = s_input[:, 4:4+(self.n_good-1)*2]
        other_good_ins = []
        for i in range(self.n_good-1):
            ...
            assert(temp1.shape[1]==2)
            other_good_ins.append(temp1)
        ...
        #food mlp
        food_input = s_input[:,-self.n_food*1:] #[None,n_food*2]

        food_input = torch.split(food_input, 1, dim=1) #split size = 2 tuple()
        # print(len(food_input))
        food_input = torch.stack(list(food_input), dim=0) #[n_food,None,1]
...
class Critic_att1(nn.Module):
    ...
    def forward(self,o,u,logger=None, iter_n=None):
        ...
        # split self obs
        length_obs = 4+self.n_food*1+(self.n_good-1)*2
        self_start = (self.index)*length_obs
