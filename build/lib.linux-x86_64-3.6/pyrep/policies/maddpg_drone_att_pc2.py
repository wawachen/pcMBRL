import torch
import os
from pyrep.networks.model_attention2 import Attention_Actor as Actor1
from pyrep.networks.model_attention2 import Attention_Critic as Critic1
import numpy as np
...
class MADDPG:
    def __init__(self, args, agent_id, rank):  # 因为不同的agent的obs、act维度可能不一样，所以神经网络不同,需要agent_id来区分
        ...
        # create the network
        self.actor_network = Actor1(args, input_size=4)
        self.critic_network = Critic1(args, agent_id, hidden_dim=128,attend_heads=4)

        # build up the target network
        self.actor_target_network = Actor1(args, input_size=4)
        self.critic_target_network = Critic1(args,agent_id, hidden_dim=128,attend_heads=4)
    ...
    def select_action(self, o, noise_rate, epsilon,use_gpu,logger):
        ...
        if np.random.uniform() < epsilon:
            ...
            inputs = torch.tensor(o, dtype=torch.float32).view(-1,o[0].shape[1])
    ...
    def train(self, transitions, other_agents, logger, use_gpu):
        ...
        o_next1 = torch.tensor(o_next).view(-1,o_next[0].shape[1])
        o_1 = torch.tensor(o).view(-1,o[0].shape[1])
        ...
        with torch.no_grad():
            ...
            for agent_id in range(self.args.n_agents):
                if agent_id == self.agent_id:
                    u_next.append(self.actor_target_network(o_next1))
                else:
                    # 因为传入的other_agents要比总数少一个，可能中间某个agent是当前agent，不能遍历去选择动作
                    u_next.append(other_agents[index].actor_target_network(o_next1)) #n*[none, adim]
                    index += 1
        ...
        u[self.agent_id] = self.actor_network(o_1)
