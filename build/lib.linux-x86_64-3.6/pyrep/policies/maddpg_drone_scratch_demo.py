...
class MADDPG:
    ...
    # update the network
    def train(self, transitions, other_agents,logger, use_gpu, epi, demo_transitions=None):
        # for key in transitions.keys():
        #     transitions[key] = torch.tensor(transitions[key], dtype=torch.float32)
        for key in transitions.keys():
            transitions[key] = torch.tensor(transitions[key], dtype=torch.float32)
            if epi<=300:
                demo_transitions[key] = torch.tensor(demo_transitions[key], dtype=torch.float32)
        ...
        if epi<=300:
            o_demo, u_demo = [],[]
            for agent_id in range(self.args.n_agents):
                o_demo.append(demo_transitions['o_%d' % agent_id])
                u_demo.append(demo_transitions['u_%d' % agent_id])
        ...
        # the actor loss
        if epi<=300:
            actor_loss = torch.sum((self.actor_network(o_demo[self.agent_id])-u_demo[self.agent_id]).pow(2),dim=1).mean()
        else:
