...
class MADDPG:
    def __init__(self, args, rank):  # 因为不同的agent的obs、act维度可能不一样，所以神经网络不同,需要agent_id来区分
        ...
        if os.path.exists(self.model_path1 + '/params.pkl'):
            self.initialise_networks(self.model_path1+'/params.pkl')
            print("wawa")
    ...
    def save_model(self, train_step):
        ...
        save_dict = {'actor_params' : self.actor_network.state_dict(),
                    ...
                    'actor_target_params': [self.actor_network.state_dict()],
                    'critic_params' : self.critic_network.state_dict(),
                    'critic_target_params' : self.critic_network.state_dict(),
