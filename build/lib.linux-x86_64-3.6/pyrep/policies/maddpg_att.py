import torch
import os
from pyrep.networks.model_attention import Actor_att as Actor
from pyrep.networks.model_attention import Critic_att as Critic
...
class MADDPG:
    def __init__(self, args, agent_id):  # 因为不同的agent的obs、act维度可能不一样，所以神经网络不同,需要agent_id来区分
        ...
        self.critic_network = Critic(args,index = agent_id)

        # build up the target network
        # print(args.action_shape[agent_id])
        self.actor_target_network = Actor(args)
        self.critic_target_network = Critic(args,index = agent_id)
