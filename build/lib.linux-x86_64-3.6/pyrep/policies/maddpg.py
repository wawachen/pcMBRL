import torch
import os
from pyrep.networks.actor_critic import Actor_t, Critic_t
...
class MADDPG:
    def __init__(self, args, agent_id):  # 因为不同的agent的obs、act维度可能不一样，所以神经网络不同,需要agent_id来区分
        ...
        # create the network
        self.actor_network = Actor_t(args, agent_id)
        self.critic_network = Critic_t(args)

        # build up the target network
        self.actor_target_network = Actor_t(args, agent_id)
        self.critic_target_network = Critic_t(args)
        ...
        if not os.path.exists(self.args.save_dir_t):
            os.mkdir(self.args.save_dir_t)
        # path to save the model
        self.model_path = self.args.save_dir_t + '/' + self.args.scenario_name_t
