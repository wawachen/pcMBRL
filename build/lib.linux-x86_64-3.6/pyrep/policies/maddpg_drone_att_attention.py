...
class MADDPG(nn.Module):
    def __init__(self, args, agent_id):  # 因为不同的agent的obs、act维度可能不一样，所以神经网络不同,需要agent_id来区分
        ...
        if self.args.is_local_obs:
            from pyrep.networks.model_attention_local import Actor_att1 as Actor1
            from pyrep.networks.model_attention_local import Critic_att1 as Critic1
        elif self.args.is_sensor_obs:
            from pyrep.networks.model_attention_sensor import Actor_att1 as Actor1
            from pyrep.networks.model_attention_sensor import Critic_att1 as Critic1
        else:
            from pyrep.networks.model_attention import Actor_att1 as Actor1
            from pyrep.networks.model_attention import Critic_att1 as Critic1
    ...
    def select_action_evaluate(self, o, noise_rate, epsilon, use_gpu,logger,niter):
        #[none, sdim]
        ...
        for i in range(o.shape[0]):
            if np.random.uniform() < epsilon:
                ...
                inputs = torch.tensor(o[i,:], dtype=torch.float32).unsqueeze(0)
                if i==0:
                    pi = self.actor_network(inputs,logger=logger, agent_id=self.agent_id, iter_n=niter).squeeze(0)
                else:
                    pi = self.actor_network(inputs).squeeze(0)
                # print('{} : {}'.format(self.name, pi))
                u = pi.cpu().numpy()
    ...
    def save_model(self, train_step):
        ...
        # if not os.path.exists(self.args.save_dir):
        #     os.makedirs(self.args.save_dir)
        # model_path = self.args.save_dir + '/' + 'agent_%d' % self.agent_id
        if not os.path.exists(self.model_path):
            os.makedirs(self.model_path)
