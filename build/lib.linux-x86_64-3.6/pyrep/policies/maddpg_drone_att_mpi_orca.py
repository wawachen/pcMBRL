...
class MADDPG(nn.Module):
    ...
    # update the network
    def train(self, transitions, other_agents, logger, use_gpu,num_epi):
        ...
        if num_epi > 0 and num_epi % self.args.save_rate == 0:
            self.save_model(num_epi)
        self.train_step += 1
    ...
    # update the network
    def train_ORCA(self, transitions, other_agents,logger, use_gpu,num_epi):
        for key in transitions.keys():
            transitions[key] = torch.tensor(transitions[key], dtype=torch.float32)
