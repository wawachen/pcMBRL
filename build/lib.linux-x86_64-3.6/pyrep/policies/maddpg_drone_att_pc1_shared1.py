...
class MADDPG:
    def __init__(self, args, rank):  # 因为不同的agent的obs、act维度可能不一样，所以神经网络不同,需要agent_id来区分
        ...
        self.model_path1 = self.model_path + '/' + 'agent'
        if not os.path.exists(self.model_path1):
            os.makedirs(self.model_path1)

        if self.args.stage == 2:
        # print(self.model_path1 + '/params.pkl')
            if os.path.exists("/home/wawa/RL_transport_3D/navigation_curriculum/stage1/model_drone2/agent" + '/params.pkl'):
                self.initialise_networks("/home/wawa/RL_transport_3D/navigation_curriculum/stage1/model_drone2/agent"+'/params.pkl')
        #     print("wawa")
